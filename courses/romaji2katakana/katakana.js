...
const randomizeKatakana = Math.floor(Math.random() * (romajiLetters.length - 0));
chosenKatakana.innerText = romajiLetters[randomizeKatakana];
...
for (let i = 0; i < 4; i++) {
    randomizer(katakanas, chooseRomajiArray);
}
chooseRomajiArray.push(katakanas[randomizeKatakana]);
const finalArray = [];
if (chooseRomajiArray.includes(katakanas[randomizeKatakana])) {
    for (let i = 0; i < 5; i++) {
...
finalArray.map((letter, index) => {
    ...
    if (finalArray[index] === katakanas[randomizeKatakana]) {
        ...
        button.addEventListener("click", () => {
            ...
            setTimeout(() => {
                ...
                notif.innerText = `The correct answer is ${katakanas[randomizeKatakana]}`;
                reload(1000);
            }, 300)
        })
    }
